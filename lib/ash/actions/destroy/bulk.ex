...
defmodule Ash.Actions.Destroy.Bulk do
  ...
  def run(domain, %Ash.Query{} = query, action, input, opts, not_atomic_reason) do
    ...
    Ash.Tracer.span :bulk_destroy,
                    ...
                    opts[:tracer] do
      Ash.Tracer.set_metadata(opts[:tracer], :bulk_destroy, fn ->
        %{
          domain: domain,
      ...
      Ash.Tracer.telemetry_span [:ash, Ash.Domain.Info.short_name(domain), :bulk_destroy],
                                fn ->
                                  %{
                                    resource_short_name:
  ...
  defp do_atomic_destroy(
    ...
    with :ok <-
      ...
      destroy_result =
        Ash.DataLayer.destroy_query(
          data_layer_query,
          atomic_changeset,
          destroy_query_opts
        )
        |> Ash.Actions.Helpers.rollback_if_in_transaction(query.resource, nil)
    ...
    if opts[:return_notifications?] do
      ...
      if notify? do
        ...
        if opts[:notify?] do
          ...
          %{bulk_result | notifications: remaining_notifications}
        else
          %{bulk_result | notifications: []}
        end
      else
