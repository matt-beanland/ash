...
defmodule Ash.Temporal do
  ...
  @type as_of :: :now | Ash.Range.t() | term() | nil

  @doc """
  Resolves an `as_of` to a point in time.

  `:now` resolves to the current time. Anything else is returned unchanged. `nil` means no
  particular time was provided. A range resolves to the point its period begins at. One with
  no lower bound has no such point.
  """
  ...
  The value comes back in the type the resource builds its periods from. A range is the
  period. Anything else begins where the write takes effect and extends forever unless a
  later write closes it.
  """
